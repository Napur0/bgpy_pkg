...
class BGPSecCryptBPO(BGPSecCrypt):
    """
        ...
        *,
        ...
        for prefix, ann_list in self.recv_q.items():
            ...
            while(len(announcement_list) > 0): ### while loop to check all valid anns for bgpsec validity
                ...
                if(self._bgpsec_verification(current_ann, self.as_.asn)):
    ...
    def process_inc_ann_list(
        ...
        if (og_ann != current_ann):
            ...
            if(not self._bgpsec_verification(current_ann, self.as_.asn)):
    ...
    def _policy_propagate(
            ...
            if isinstance(neighbor.policy, BGPSecCryptBPO):
